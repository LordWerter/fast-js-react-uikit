...
export const SelectorBtn: React.FC<Props> = (props): JSX.Element => {
    ...
    const { CWrap, CurOptionWrap, CurOption, ToggleBtn, OptionsWrap } =
        genFCElems(SelectorBtnElems);
    const theme = getFCTheme({
        FCName: 'SelectorBtn',
        typeToken,
        nodeNames: ['CWrap', 'CurOption', 'ToggleBtn', 'OptionsWrap'],
        customize,
    });

    const testIds = {
        CWrap: getElemNodeCST(`SELECTOR_BTN__CWRAP`, hlaToken, actionToken),
        CurOptionWrap: getElemNodeCST(
            `SELECTOR_BTN__CUR_OPTION_WRAP`,
            hlaToken,
            actionToken
        ),
        CurOption: getElemNodeCST(
            `SELECTOR_BTN__CUR_OPTION`,
            hlaToken,
            actionToken
        ),
        ToggleBtn: getElemNodeCST(
            `SELECTOR_BTN__TOGGLE_BTN`,
            hlaToken,
            actionToken
        ),
        OptionsWrap: getElemNodeCST(
            `SELECTOR_BTN__OPTIONS_WRAP`,
            hlaToken,
            actionToken
        ),
    };
    ...
    return (
        <CWrap
            ...
            <CurOptionWrap
                sizeId={sizeId}
                theme={theme.CurOptionWrap}
                data-testid={testIds.CurOptionWrap}>
                <CurOption
                    sizeId={sizeId}
                    theme={theme.CurOption}
                    data-testid={testIds.CurOption}>
                    {caption}
                </CurOption>
                <ToggleBtn
                    sizeId={sizeId}
            ...
            <OptionsWrap
                className={showOptions ? 'isVisible' : ''}
                sizeId={sizeId}
                theme={theme.OptionsWrap}
                data-testid={testIds.OptionsWrap}>
