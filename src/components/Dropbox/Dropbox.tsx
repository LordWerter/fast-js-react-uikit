...
export interface DropboxItem {
    key: string;
    text: string;
    handleOnClick: (event: React.MouseEvent<HTMLButtonElement>, data?: unknown) => unknown;
}
...
export const Dropbox: React.FC<Props> = (props): JSX.Element => {
    const { defaultItemKey, items, sizeId = 'mobile', customize = {}, typeToken = null, hlaToken, actionToken } = props;
    ...
    const theme = getFCTheme({
        ...
        nodeNames: ['CWrap', 'HeaderBtn', 'ItemsWrap', 'Item'], customize
    });
    ...
    const testIds = {
        ...
        ItemsWrap: getElemNodeCST(`SWITCH_BTN__Items_Wrap`, hlaToken, actionToken),
        Item: getElemNodeCST(`SWITCH_BTN__Item`, hlaToken, actionToken),
    };

    return (
        <CWrap sizeId={sizeId} theme={theme.CWrap} data-testid={testIds.CWrap}>
            <HeadBtn sizeId={sizeId} theme={theme.HeaderBtn} data-testid={testIds.HeadBtn} onClick={() => setShowItems(true)}>
                {items[selectedItemKey].text}
            </HeadBtn>
            <ItemsWrap sizeId={sizeId} theme={theme.ItemsWrap} data-testid={testIds.ItemsWrap} className={showItems ? 'isVisible' : ''} onClick={() => setShowItems(false)}>
                {Object.keys(items).map((itemKey: any) => {
                    ...
                    return (
                        <Item
                            ...
                            className={key === selectedItemKey ? 'isSelected' : ''}
